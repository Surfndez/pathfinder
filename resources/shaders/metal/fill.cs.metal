...
static inline __attribute__((always_inline))
float4 calculateFillAlpha(thread const int2& tileSubCoord, thread const uint& tileIndex, const device bFillTileMap& v_149, const device bFills& v_173, thread texture2d<float> uAreaLUT, thread const sampler uAreaLUTSmplr, const device bNextFills& v_254)
{
    int fillIndex = v_149.iFillTileMap[tileIndex];
    if (fillIndex < 0)
    {
        return float4(0.0);
    }
    ...
    do
    {
        uint2 fill = v_173.iFills[fillIndex];
        ...
        fillIndex = v_254.iNextFills[fillIndex];
    } while (fillIndex >= 0);
    return coverages;
}

static inline __attribute__((always_inline))
int2 calculateTileOrigin(thread const uint& tileIndex)
{
    return int2(int(tileIndex & 255u), int((tileIndex >> 8u) & 255u)) * int2(16);
}

kernel void main0(constant int& uFirstTileIndex [[buffer(3)]], const device bFillTileMap& v_149 [[buffer(0)]], const device bFills& v_173 [[buffer(1)]], const device bNextFills& v_254 [[buffer(2)]], texture2d<float> uAreaLUT [[texture(0)]], texture2d<float, access::write> uDest [[texture(1)]], sampler uAreaLUTSmplr [[sampler(0)]], uint3 gl_LocalInvocationID [[thread_position_in_threadgroup]], uint3 gl_WorkGroupID [[threadgroup_position_in_grid]])
{
    ...
    uint param_1 = tileIndex;
    float4 coverages = calculateFillAlpha(param, param_1, v_149, v_173, uAreaLUT, uAreaLUTSmplr, v_254);
    uint param_2 = tileIndex;
    int2 tileOrigin = calculateTileOrigin(param_2);
