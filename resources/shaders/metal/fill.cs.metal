...
kernel void main0(constant int& uFirstTileIndex [[buffer(0)]], constant int& uBinnedOnGPU [[buffer(3)]], const device bFillTileMap& _150 [[buffer(1)]], const device bFills& _173 [[buffer(2)]], const device bTiles& _249 [[buffer(4)]], texture2d<float> uAreaLUT [[texture(0)]], texture2d<float, access::write> uDest [[texture(1)]], sampler uAreaLUTSmplr [[sampler(0)]], uint3 gl_LocalInvocationID [[thread_position_in_threadgroup]], uint3 gl_WorkGroupID [[threadgroup_position_in_grid]])
{
    ...
    do
    {
        uint fillFrom = _173.iFills[(fillIndex * 3) + 0];
        uint fillTo = _173.iFills[(fillIndex * 3) + 1];
        ...
        fillIndex = int(_173.iFills[(fillIndex * 3) + 2]);
        iteration++;
    } while ((fillIndex >= 0) && (iteration < 1024));
    uint alphaTileIndex;
    if (uBinnedOnGPU != 0)
    {
        alphaTileIndex = uint(_249.iTiles[(tileIndex * 4u) + 1u]);
    }
    else
    {
