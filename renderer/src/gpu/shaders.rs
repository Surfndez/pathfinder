...
pub struct AlphaTileCopyProgram<D> where D: Device {
    pub alpha_tile_program: AlphaTileProgram<D>,
    pub src_uniform: D::Uniform,
}

impl<D> AlphaTileCopyProgram<D> where D: Device {
    pub fn new(device: &D, resources: &dyn ResourceLoader) -> AlphaTileCopyProgram<D> {
        let alpha_tile_program = AlphaTileProgram::from_fragment_shader_name(device,
                                                                             resources,
                                                                             "tile_alpha_copy");
        let src_uniform = device.get_uniform(&alpha_tile_program.program, "Src");
        AlphaTileCopyProgram { alpha_tile_program, src_uniform }
    }
}
